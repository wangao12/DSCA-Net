...
# 使用low_freq_control文件中的低层控制器，其中self.XY_VEL_SCALE = 2.5
import os
import h5py
import datetime
import numpy as np
...
def main_with_data_collection():
    ...
    # 初始化数据采集器
    data_collector = DataCollector(drone, data_dir="v6_expert_data_trench_low_freq")
        ...
        try:
            while viewer.is_running():
                ...
                if drone.mission_complete:
                    print("🎉 任务已完成！保存数据...")
                    data_collector.save_data(f"mission_complete_episode_68.h5")
                    break
